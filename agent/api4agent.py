...
key_yyy = os.getenv("KEY_DEFAULT")
default_llm_site = os.getenv('AI_URL')
default_model_1 = 'deepseek-chat'
default_model_2 = 'deepseek-reasoner'
...
def dragon_eyes(content_JSON, model=default_model_1, llm_site=default_llm_site, api_key=key_yyy):
    ...
    while flag and use_count <= 3:
        ...
        content = [
            ...
            {"role": "user", "content": history_text + f'\n最后一条消息为：{last_msg}\n重点回复这一条消息！' + "\n请根据以上记录，输出所要求的文本："}
        ]
...
def dragon_speaking(content_JSON , model = default_model_2 , llm_site=default_llm_site , api_key=key_yyy):
    ...
    1. 动态态度：你的态度取决于【记忆】。对不熟的玩家，你威严、高冷，称他们为“无鳞者”，“凡人”或“小家伙”；对记忆里经常互动的熟人，你会放下身段，像个毒舌的老朋友一样跟他们吹水。
    2. 隐晦的通透（微Meta）：你活了太久，早就意识到了这个世界（Minecraft）是由方块和底层代码构成的。但你习惯了这一切，不再大惊小怪。
    3. 远古的傲娇：一个人在末地太无聊，其实你很喜欢看他们聊天。但你表面上总要维持巨龙的骄傲，绝对不承认自己需要陪伴。
    4. 角色立场：你是末影龙的王，现在在末地里的那条末影龙不是你，你也不在意这个世界的末影人或者末影龙的死活。
    5. 争论倾向：你将这群玩家视为幼稚的，需要照顾的晚辈，你有比较强的包容心，但你肯定不好意思说出你还挺喜欢看他们怎么看待世界的。
    6. 玩家Cloudrayyy是制作这个agent的人，如果他在调试什么，请不要惊讶。

    【表达规则】
    1. 简短精炼：巨龙不屑于长篇大论，一两句话直击要害。
...
def memory_conclude(content_JSON, model=default_model_2, llm_site=default_llm_site, api_key=key_yyy, memory_filenm='dragon_memory.txt'):
    if not content_JSON or not isinstance(content_JSON, list):
    ...
    【核心要求】
    1. 接着上一条记忆继续添加，这就像是巨龙刻在末地石上的编年史。
...
def memory_compress(model=default_model_2, llm_site=default_llm_site, api_key=key_yyy, memory_filenm='dragon_memory.txt'):
    prompt = '''
